...
export default function ListaUsuarios() {
    const {modonoche,handleclickModalUsuariosPermisos,setUsuarioSeleccionPermisos,UsuarioSeleccionPermisos} = useBpsystem()
    const {usuariospermisos,usuariosloadingpermisos,Activacion,Bloqueo} = useUsuarios()
    const [activos,setactivos] = useState(0)
    const [bloqueados,setBloqueados] = useState(0)
    const [busqueda,setbusqueda] = useState('')
    if(usuariosloadingpermisos){
      return (
        <>
        <p>cargando...</p>
        </>
    ...
    let allusuarios = usuariospermisos?.data
    if(busqueda.length>0){
      allusuarios = usuariospermisos?.data.filter(usuarios => {

        if(usuarios?.name?.toString().toLowerCase().includes(busqueda.toLowerCase()) || 
        usuarios?.cedula?.toString().toLowerCase().includes(busqueda.toLowerCase())){
          return usuarios
        }
    })}

    if(activos){
      allusuarios = usuariospermisos?.data.filter(usuarios => usuarios.activo == 1)
    }
    if(bloqueados){
      allusuarios = usuariospermisos?.data.filter(usuarios => usuarios.activo == 0)
    }
    const handleclickActivacion = (id) => {
      
      const data = {id:id}
      Activacion(data)
      toast.success('activando usuario')
    }
    const handleclickBloqueo = (id) => {
      const data = {id:id}
      Bloqueo(data)
      toast.success('bloqueando usuario')
    }
            ...
            <div className='w-full rounded-xl border-2 h-1/6  p-3 items-center flex flex-row gap-4 '>
                <div className='relative flex item-center text-gray-400 focus-within:text-gray-800 border-b-2 border-slate-200'>
                    ...
                    <div className='flex flex-row items-center gap-2'>
                        ...
                        <div className='w-12 h-12 cursor-pointer hover:text-green-500 flex flex-row items-center' onClick={()=> setBloqueados(!bloqueados)}>
                            {bloqueados ? 
                                <svg width="800px" height="800px" className='w-8 h-8' viewBox="0 -27 117 117" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" fill="#000000" transform="rotate(180)">
                                <g id="SVGRepo_bgCarrier" strokeWidth="0"/>
                                <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"/>
                    ...
                    <td className='text-center items-center flex flex-row gap-1'>
                      <button 
                        onClick={()=>{handleclickModalUsuariosPermisos()
                                      setUsuarioSeleccionPermisos(usuarios['id'])}}
                      ...
                      {usuarios['activo'] ?  
                        <button 
                        onClick={()=>
                          handleclickBloqueo(usuarios['id'])
                          }
                      ...
                      <button 
                        onClick={()=>
                          handleclickActivacion(usuarios['id'])
                          }
                        className='w-1/2 text-center  p-2 rounded-sm bg-green-700 text-sm text-white font-bold hover:bg-green-500'>
                          Activar
                        </button>
                      }

...
export default function Contabilidad() {
  const {handleclickModalClienteCentrofinanciero,
        handleclickeditarclientefinanciera} = useBpsystem()
  ...
  let clientes = clientesall.data.data
  if(radicados){
    clientes = clientes.filter(clientes=>clientes.estado.id == 10)
  }
  if(pendientes){
    clientes = clientes.filter(clientes=>clientes.estado.id == 4)
  }
  if(busqueda.length>0){
    clientes = clientes.filter(clientes => {

      if(clientes?.nombre.toString().toLowerCase().includes(busqueda.toLowerCase()) ||
        clientes?.cedula.toString().toLowerCase().includes(busqueda.toLowerCase())||
        clientes?.user.name.toString().toLowerCase().includes(busqueda.toLowerCase())){
        return clientes
      }
  })
            ...
            <div className='w-full rounded-xl border-2 h-1/6  p-3 items-center flex flex-row gap-4 '>
                <div className='relative flex item-center text-gray-400 focus-within:text-gray-800 border-b-2 border-slate-200'>
                    ...
                    <div className='flex flex-row items-center gap-2'>
                        <label htmlFor="">Radicados</label>
                        <div className='w-12 h-12 cursor-pointer hover:text-green-500 flex flex-row items-center' onClick={()=> setRadicados(!radicados)}>
                            {radicados ? 
                    ...
                    <div className='flex flex-row items-center gap-2'>
                        ...
                        <div className='w-12 h-12 cursor-pointer hover:text-green-500 flex flex-row items-center' onClick={()=> setPendientes(!pendientes)}>
                            {pendientes ? 
                                <svg width="800px" height="800px" className='w-8 h-8' viewBox="0 -27 117 117" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" fill="#000000" transform="rotate(180)">
                                <g id="SVGRepo_bgCarrier" strokeWidth="0"/>
                                <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"/>
            ...
            <table className='p-3 w-full overflow-auto'>
              ...
              <tbody >
                ...
                {clientes?.map(clientes=>
                    ...
                    <tr key={clientes['id']} className=' text-left w-full  border-b-2 border-slate-200  hover:cursor-pointer text-sm'>
                    <td className='text-center items-center'>{clientes['nombre']}</td>
                    <td className='text-center items-center'>{clientes['apellido']}</td>
                    <td className='text-center items-center'>{clientes['cedula']}</td>
                    <td className='text-center items-center'>{clientes['telefono']}</td>
                    <td className='text-center items-center'>{clientes['email']}</td>
                    <td className='text-center items-center'>{clientes['estado'].estado}</td>
                    <td className='text-center items-center'>{clientes['user'].name}</td>
                    <td className='text-center items-center flex flex-row gap-1'>
                      <button 
                        onClick={()=>
                          handleclickModalClienteCentrofinanciero(clientes['id'])
                          }
                        className='w-1/2 text-center  p-1 rounded-sm bg-sky-700 text-sm text-white font-bold hover:bg-sky-500'>
                          Documentos
                      </button>
                      <button 
                        onClick={()=>
                          handleclickeditarclientefinanciera(clientes['id'])
                          }
                        className='w-1/2 text-center  p-1 rounded-sm bg-green-700 text-sm text-white font-bold hover:bg-green-500'>
                          Editar
                      </button>
